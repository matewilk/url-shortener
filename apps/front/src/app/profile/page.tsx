...
export default withCapabilities(async ({ capabilities }) => {
  const user = await capabilities.users.getUser();
  if (!user) {
    return redirect("/login");
  }

  const urls = await capabilities.urls.urls();

  // TODO: Replace with actual click data from the API
